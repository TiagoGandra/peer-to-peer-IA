import socket
import threading
from detecçãodefakenews import (
    manual_testing,
)
...
def handle_client(client_socket, client_address, peer_sockets):
    while True:
        ...
        # Executa o modelo de IA com a mensagem recebida do cliente
        result = manual_testing(data)

        # Envia o resultado do modelo para todos os clientes conectados
        for peer_socket in peer_sockets:
            if peer_socket != client_socket:
                peer_socket.send(result.encode("utf-8"))
